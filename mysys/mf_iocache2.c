...
/**
  The same as above but copying is made in so many steps as the number of
  fragments. Each step is wrapped with writing to the file @c
  before_frag and @c after_frag formated strings.
  @c after_total string is copied as the final item. 
*/
int
                       ...
                       const char* after_total)
{
  ...
  */
  ...
  for (i= 0; i < n_frag; i++, total_written += frag_written)
  {
       frag_written= 0;
